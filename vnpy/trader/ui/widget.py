...
SETTING_HELP_TEXT: dict[str, str] = {
    ...
    "news.ingestion.symbols": "新闻入库关注标的列表，逗号分隔，例如 600519.SSE,000001.SZSE；留空时不做全市场逐股暴力拉取，会先跑全局/公告类来源，并在配置 news.entity.catalog_path 后按股票目录分批轮询。",
    "news.ingestion.symbol_source": "symbols 留空时的股票来源；默认 auto：优先使用 news.entity.catalog_path 中的股票目录，没有目录则只跑全局新闻/公告。",
    "news.ingestion.symbol_batch_size": "symbols 留空且配置股票目录时，每轮最多轮询的股票数量；默认 50，用于慢慢拉全市场个股新闻，避免免费源限流。",
