...
SETTING_HELP_TEXT: dict[str, str] = {
    ...
    "news.ingestion.enabled": "外部新闻入库定时任务开关，默认开启。开启后按 news.ingestion.providers 拉取新闻并写入 PostgreSQL；公开源失败只会降级，不影响主交易链路。",
    "news.ingestion.providers": "外部新闻 provider 顺序。生产建议 cninfo_announcement,sse_announcement,gdelt_global_news,akshare_stock_news；CNINFO/上交所为官方公告，GDELT 为免费全球宏观新闻，AKShare 仅低成本补充。",
    ...
    "news.ingestion.symbols": "新闻入库关注标的列表，逗号分隔，例如 600519.SSE,000001.SZSE；留空时不做一次性全市场暴力拉取，会按 news.entity.catalog_path、vn.py 已缓存合约、AKShare A 股列表依次生成股票池并分批轮询。",
    "news.ingestion.symbol_source": "symbols 留空时的股票来源；默认 auto：优先使用 news.entity.catalog_path，其次复用 vn.py 合约表，最后懒加载 AKShare A 股列表；global_only 表示只跑全局新闻/公告。",
    "news.ingestion.symbol_batch_size": "symbols 留空且自动生成股票池时，每轮最多轮询的股票数量；默认 50，用于慢慢拉全市场个股新闻，避免免费源限流。",
    ...
    "news.ingestion.gdelt_query": "GDELT DOC 2.0 公共 API 查询表达式；用于宏观/海外/产业链事件，不建议按全市场个股暴力查询。",
    ...
    "financial.ingestion.enabled": "完整财报自动入库总开关，默认开启。vn.py 启动时注册 scheduler，并在盘后/早晨按配置拉取财报；关闭后自动任务暂停。",
    "financial.ingestion.providers": "财报 provider 顺序。默认 akshare_sina,akshare_eastmoney,akshare_indicator,cninfo_report,exchange_report：结构化报表用 AKShare，官方 PDF 元数据用巨潮/交易所。",
    "financial.ingestion.symbols": "财报入库关注标的列表，逗号分隔；留空时按 financial.ingestion.catalog_path、news.entity.catalog_path、vn.py 已缓存合约、AKShare A 股列表依次生成股票池并分批慢速回补，不在启动时全市场拉取。",
    "financial.ingestion.catalog_path": "财报入库股票池 CSV/JSON 路径；留空时复用 news.entity.catalog_path，再退回 vn.py 合约和 AKShare A 股列表。",
    "financial.ingestion.lookback_years": "财报历史回补年限；默认 5 年。自动任务只做增量，历史回补由用户手动触发。",
    "financial.ingestion.schedule": "盘后财报增量任务时间，HH:MM；默认 20:30。",
    "financial.ingestion.morning_retry_enabled": "是否启用次日早晨轻量补偿；默认开启，用于补晚间披露和前一轮失败。",
    "financial.ingestion.morning_retry_schedule": "早晨补偿任务时间，HH:MM；默认 08:30。",
    "financial.ingestion.full_market_backfill_enabled": "是否允许全市场历史回补；默认 false，避免公开接口限流。",
    "financial.ingestion.max_workers": "财报拉取并发数；默认 2，免费公开源不建议调高。",
    "financial.ingestion.request_timeout": "单个财报 provider 请求超时时间，单位秒；默认 30。",
    "financial.ingestion.symbol_batch_size": "全市场或大股票池慢速轮询时每轮最多处理的股票数；默认 20。",
    "financial.context.max_statement_periods": "喂给 TradingAgents 的最多报告期数量；默认 4。",
    "financial.context.require_announced_only": "历史分析是否只使用已公告财报；默认 true，避免回测穿越。",
    "news.entity.catalog_path": "股票实体目录 CSV/JSON 路径，用于名称、简称、代码、别名识别；启用官方公告生产链路时建议必填。",
    "news.entity.refresh_interval_hours": "股票实体目录刷新间隔，单位小时；默认 24 小时。",
    "news.filter.min_trust_score": "TradingAgents 新闻上下文最低可信度；默认 0.70，官方公告可优先保留。",
    "news.filter.min_link_confidence": "新闻关联到具体股票的最低置信度；默认 0.75。",
    "news.filter.max_items_per_symbol": "每个股票单次喂给 TradingAgents 的新闻/公告摘要上限；默认 20。",
    "news.filter.allowed_event_types": "允许进入 TradingAgents 上下文的事件类型，例如 announcement,earnings,regulatory,buyback,holding_change,industry,macro。",
    "news.filter.intraday_window_days": "日内/分时分析默认新闻窗口天数；默认 3 天。",
    "news.filter.research_window_days": "长期研究/复盘默认新闻窗口天数；默认 180 天。",
    "news.llm_classifier.enabled": "LLM 消息语义分类/行业板块股票关联开关，默认关闭；开启后只作为规则分类器的增强，不直接下单。",
    "news.llm_classifier.model": "LLM 消息分类模型；智谱建议 glm-4.7。",
    "news.llm_classifier.api_key_env_var": "LLM 消息分类 API key 环境变量名；智谱填写 ZHIPU_API_KEY，真实 key 仍走安全输入或系统环境变量。",
    "news.llm_classifier.intraday_thinking_type": "日内/分时消息分类 Thinking 开关；默认 disabled，保证低延迟。",
    "news.llm_classifier.scheduled_thinking_type": "定时新闻入库消息分类 Thinking 开关；默认 disabled，避免拖慢入库任务。",
    "news.llm_classifier.research_thinking_type": "长期研究消息分类 Thinking 开关；默认 enabled。",
    "news.llm_classifier.replay_thinking_type": "复盘/回测消息分类 Thinking 开关；默认 enabled。",
    "news.llm_classifier.batch_thinking_type": "批量行业映射消息分类 Thinking 开关；默认 enabled。",
    "news.llm_classifier.fast_timeout_seconds": "不开 Thinking 的 LLM 消息分类超时时间，单位秒；默认 360 秒。",
    "news.llm_classifier.deep_timeout_seconds": "开 Thinking 的 LLM 消息分类超时时间，单位秒；默认 2700 秒（45 分钟）。",
    "news.llm_classifier.min_confidence": "LLM 提名股票写入 event_symbol_link 的最低置信度；默认 0.65，后续 Toolkit 仍会按更高阈值过滤。",
}
