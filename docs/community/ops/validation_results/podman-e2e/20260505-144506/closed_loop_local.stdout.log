...
| pytest_non_alpha | failed | yes | `uv run --with pytest --with psycopg2-binary pytest tests -q --ignore=tests/test_alpha101.py` | command failed |
...
| readiness_cli | passed | yes | `uv run --with psycopg2-binary vnpy-tradingagents-schema readiness --json` | command passed |
| tradingagents_worker_factory_env | passed | yes | `env:TRADINGAGENTS_WORKER_FACTORY` | context-only worker factory is configured |
| llm_api_key_env | passed | yes | `env:OPENAI_API_KEY` | LLM API key environment variable is configured |
...
- ` M tools/production/closed_loop_validation.py`
- ` M vnpy_tradingagents/readiness.py`
- `?? docs/community/ops/podman_e2e_test_cases.md`
- `?? docs/community/ops/validation_results/podman-e2e/`
- `?? tests/fixtures/`
- `?? tools/podman/`

## 结论

本次验证未达到生产可用准入；未通过或阻塞的生产门禁：pytest_non_alpha, worktree_clean
