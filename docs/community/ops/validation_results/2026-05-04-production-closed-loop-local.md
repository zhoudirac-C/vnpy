...
| postgres_dsn_env | blocked | yes | `env:QUANT_DATABASE_URL` | QUANT_DATABASE_URL is not configured in this validation environment |
...
本次验证未达到生产可用准入；未通过或阻塞的生产门禁：readiness_cli, postgres_dsn_env, tradingagents_worker_factory_env, llm_api_key_env, worktree_clean
